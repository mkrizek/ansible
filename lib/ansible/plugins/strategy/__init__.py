        ...
        if meta_action == 'noop':
            ...
            if _evaluate_conditional(target_host):
                for host in self._inventory.get_hosts(iterator._play.hosts):
                    ...
                    iterator.set_fail_state_for_host(host.name, iterator.FAILED_NONE)
            ...
            if _evaluate_conditional(target_host):
                for host in self._inventory.get_hosts(iterator._play.hosts):
                    if host.name not in self._tqm._unreachable_hosts:
                        iterator.set_run_state_for_host(host.name, iterator.ITERATING_COMPLETE)
                msg = "ending play"
            else:
                skipped = True
            ...
            if _evaluate_conditional(target_host):
                iterator.set_run_state_for_host(target_host.name, iterator.ITERATING_COMPLETE)
